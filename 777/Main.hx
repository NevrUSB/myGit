...
var count:Int = 5;
	...
	var maxXPosition:Int = Math.round(Browser.window.innerWidth); //вычисляем ширину окна
	var maxYPosition:Int = Math.round(Browser.window.innerHeight); // вычисляем высоту окна
...
public function new (x1:Int, y1:Int, x2:Int, y2:Int, myParent:Int){
	...
	addEventListener(MouseEvent.MOUSE_DOWN, function(e){trace(this.number+" -> "+this.myParent); e.stopPropagation(); });
	...
	graphics.beginFill(Math.round(Math.random() * 0xFFFFFF));// рандомный цвет квадратов
